...
func (bc *testBlockChain) StateAt(common.Hash) (StateDB, error) {
...
func setupTxPool() (*TxPool, *ecdsa.PrivateKey) {
	statedb, _ := stateNew(common.Hash{}, state.NewDatabase(rawdb.NewMemoryDatabase()), nil)
...
func (c *testChain) State() (StateDB, error) {
	// delay "state change" by one. The tx pool fetches the
	// state multiple times and by delaying it a bit we simulate
	// a state change between those fetches.
	stdb := c.statedb
	if *c.trigger {
		c.statedb, _ = stateNew(common.Hash{}, state.NewDatabase(rawdb.NewMemoryDatabase()), nil)
